                ...
                for (Object o : chunkstoremove.values()) {
                    if (chunkCoord == null) {
                        ...
                        for(Field ff : f) {
                            ...
                            else if((nbtTag == null) && (ff.getType().equals(CompoundNBT.class))) {
                                nbtTag = ff;
        ...
        while (iter.hasNext()) {
            ...
            // If chunk is loaded and not being unloaded, we're grabbing its NBT data
                ...
                if (vis) {  // If visible 
                    ...
                    try {
                        writechunktonbt.invoke(cps.chunkLoader, cps.getChunk(chunk.x, chunk.z, ChunkStatus.FULL, false), w, nbt);
                    } catch (IllegalAccessException e) {
