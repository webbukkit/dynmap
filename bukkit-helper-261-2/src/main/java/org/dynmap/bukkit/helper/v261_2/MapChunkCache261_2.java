...
public class MapChunkCache261_2 extends GenericMapChunkCache {
    ...
    public int getFoliageColor(BiomeMap bm, int[] colormap, int x, int z) {
        return bm.<Biome>getBiomeObject().map(Biome::getFoliageColor).orElse(colormap[bm.biomeLookup()]);
    }

    @Override
    public int getGrassColor(BiomeMap bm, int[] colormap, int x, int z) {
        return bm.<Biome>getBiomeObject().map(b -> b.getGrassColor(x, z)).orElse(colormap[bm.biomeLookup()]);
    }
}
